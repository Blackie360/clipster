...
```sh
curl -fsSL .../install.sh | sh -s -- --version 0.1.2   # pin a release
...
```sh
gh release download v0.1.2 --pattern '*_amd64.deb'   # or *_arm64.deb
sudo dpkg -i clipster_0.1.2_amd64.deb
```
...
```sh
gh release download v0.1.2 --pattern '*x86_64-unknown-linux-musl.tar.gz'
tar xzf clipster-0.1.2-x86_64-unknown-linux-musl.tar.gz
cd clipster-0.1.2-x86_64-unknown-linux-musl
